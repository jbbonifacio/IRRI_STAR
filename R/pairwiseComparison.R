  	...
  	pwTestAvailable <- c("LSD", "duncan", "HSD", "SNK", "scheffe")
     	...
     	if (length(strsplit(effect, split = ":")[[1]]) == 1) {
      	if (is.null(pwTest)) { if (nlevels(data[,effect]) <= 5) { pwTestChoice <- "LSD" } else { pwTestChoice <- "HSD" }}
          	...
          	if (designChoice <= 3) { returnData <- STAR::pairwiseAmong(data, respvar, typeTest = pwTestChoice, trmt = effect, environment = environment, dfError = aovTable[[1]][nrow(aovTable[[1]]),1], MSError = aovTable[[1]][nrow(aovTable[[1]]),3], siglevel, rdata = returnData, analysisId = analysisId)
          	} else {
            	start <- 1
               	while (is.na(match(effect, trimws(rownames(aovTable[[start]][[1]])))) && start <= length(aovTable)) start <- start + 1
               	returnData <- STAR::pairwiseAmong(data, respvar, typeTest = pwTestChoice, trmt = effect, environment = environment, dfError = aovTable[[start]][[1]][nrow(aovTable[[start]][[1]]),1], MSError = aovTable[[start]][[1]][nrow(aovTable[[start]][[1]]),3], siglevel, rdata = returnData, analysisId = analysisId)
          	}
     	}
						...
						cat(sub.label1,"\n\n")
                              	if (designChoice <= 3) {
                              	  returnData <- STAR::pairwiseWithin(data, respvar, environment = environment, typeTest = pwTestChoice[k], nobs1 = nlevels(data[,tempFactor[j]]), nobs2 = nlevels(data[,temp.blevel.max]), dfError = aovTable[[1]][nrow(aovTable[[1]]),1], MSError = aovTable[[1]][nrow(aovTable[[1]]),3], f1 = tempFactor[j], f2 = temp.blevel.max, f3 = sub.label, siglevel = siglevel, rdata = returnData, analysisId = analysisId)
                              		...
                              		if (length(unique(tempPos)) > 1) {
                              		  returnData <- STAR::pairwiseWithin(data, respvar, environment = environment, typeTest = pwTestChoice[k], nobs1 = nlevels(data[,tempFactor[j]]), nobs2 = nlevels(data[,temp.blevel.max]), dfError = df[[j]], MSE[[j]], f1 = tempFactor[j], f2 = temp.blevel.max, f3 = sub.label, siglevel = siglevel, rdata = returnData, analysisId = analysisId)
                                   		} else {
                                   		  returnData <- STAR::pairwiseWithin(data, respvar, environment = environment, typeTest = pwTestChoice[k], nobs1 = nlevels(data[,tempFactor[j]]), nobs2 = nlevels(data[,temp.blevel.max]), dfError = df[[1]], MSE[[1]], f1 = tempFactor[j], f2 = temp.blevel.max, f3 = sub.label, siglevel = siglevel, rdata = returnData, analysisId = analysisId)
                                  		}
                              	}
						cat("\n")
             		...
             		for (k in (1:length(pwTestChoice))) {
                    		if (designChoice <= 3) {
                    		  returnData <- STAR::pairwiseWithin(data, respvar, environment = environment, typeTest = pwTestChoice[k], nobs1 = nlevels(data[,tempFactor[j]]), nobs2 = nlevels(data[,atLevel[j]]), dfError = aovTable[[1]][nrow(aovTable[[1]]),1], MSError = aovTable[[1]][nrow(aovTable[[1]]),3], f1 = tempFactor[j], f2 = atLevel[j], siglevel = siglevel, rdata = returnData, analysisId = analysisId)
                         		...
                         		if (length(unique(tempPos)) > 1) {
                         		  returnData <- STAR::pairwiseWithin(data, respvar, environment = environment, typeTest = pwTestChoice[k], nobs1 = nlevels(data[,tempFactor[j]]), nobs2 = nlevels(data[,atLevel[j]]), dfError = df[[j]], MSError = MSE[[j]], f1 = tempFactor[j], f2 = atLevel[j], siglevel = siglevel, rdata = returnData, analysisId = analysisId)
                              	} else {
                              	  returnData <- STAR::pairwiseWithin(data, respvar, environment = environment, typeTest = pwTestChoice[k], nobs1 = nlevels(data[,tempFactor[j]]), nobs2 = nlevels(data[,atLevel[j]]), dfError = df[[1]], MSError = MSE[[1]], f1 = tempFactor[j], f2 = atLevel[j], siglevel = siglevel, rdata = returnData, analysisId = analysisId)
                              	}
                      		}
					cat("\n")
                    	} ## END FOR STMT -- for (k in (1:length(pwTestChoice)))
