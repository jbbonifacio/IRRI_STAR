# --------------------------------------------------------
# RCropStat Beta Menu
# Functions obtained/copyied from package nortest
# --------------------------------------------------------
...
lillie.test <- function (x) {
    ...
    RVAL <- list(statistic = c(D = K), p.value = pvalue, method = "Lilliefors (Kolmogorov-Smirnov) normality test", data.name = DNAME)
    class(RVAL) <- "htest"
    return(RVAL)
}
