...
leveneTest <- function (y, ...) { UseMethod("leveneTest") }

leveneTest.default <- function (y, group, center=median, ...) { # original levene.test
	if (!is.numeric(y)) stop(deparse(substitute(y)), " is not a numeric variable")
	if (!is.factor(group)) {
...
leveneTest.lm <- function(y, ...) { leveneTest.formula(formula(y), data=model.frame(y), ...) }
