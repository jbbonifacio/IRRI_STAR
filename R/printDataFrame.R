                              ...
                              if (length(tempsubdata) == 0) {
                                   ...
                                   if (length(tempsubdata) == 0) {
                                        ...
                                        if (all(tempsubdata < 1) && all(tempsubdata > -1)) {
                                             if (is.null(digits)) { colWidth <- c(colWidth, max(3, nchar(as.character(dataChar[i,1])), max(nchar(round(dataFrame[,i],0)), na.rm = TRUE) + 5, na.rm = TRUE) + 2)
                                             } else { colWidth <- c(colWidth, max(3, nchar(as.character(dataChar[i,1])), max(nchar(round(dataFrame[,i],0)), na.rm = TRUE) + digits, na.rm = TRUE) + 2) }
                                             #colWidth <- c(colWidth, max(nchar(as.character(dataChar[i,1])), max(nchar(round(dataFrame[,i],0))) + 5) + 1)
                                             onlyDecimal <- c(onlyDecimal, i)
                                        } else {
                                             if (all(STAR::is.wholenumber(tempsubdata))) {
                                                  dataChar[i,2] <- "integer"
                                                  colWidth <- c(colWidth, max(3, nchar(as.character(dataChar[i,1])), nchar(max(tempsubdata, na.rm = TRUE)), na.rm = TRUE) + 2)
                                             } else {
                                                  if (is.null(digits)) { colWidth <- c(colWidth, max(3, nchar(as.character(dataChar[i,1])), max(nchar(round(dataFrame[,i],0)), na.rm = TRUE) + 3, na.rm = TRUE) + 2)
                                                  } else { colWidth <- c(colWidth, max(3, nchar(as.character(dataChar[i,1])), max(nchar(round(dataFrame[,i],0)), na.rm = TRUE) + digits, na.rm = TRUE) + 2) }
                                             }
                                        }
                                   }
