...
AOVTest <- function(data, design, respvar, factor1, factor2 = NULL, factor3 = NULL,
...
#                             set = NULL, descriptive = FALSE, normality = FALSE,
#                             homogeneity = FALSE, pwTest = NULL, pwVar = NULL,
#                             contrastOption = NULL, sig = 0.05, outputPath = NULL) {
  ...
  availableDesign <- c("CRD", "RCBD", "LSD", "SplitCRD",
                       "SplitRCBD", "SplitLSD", "Strip",
                       "Split2CRD", "Split2RCBD", "Split2LSD",
                       "Strip-Split", "Split3CRD", "Split3RCBD",
                       "Split3LSD", "Strip-Split2")
  ...
  if(is.null(returnData$metrics)){
    ...
    returnData$metrics <- provMet
  }
  if(is.null(returnData$predictions)){
    provMet <- as.data.frame(matrix(nrow=0, ncol=13))
    colnames(provMet) <- c("module", "analysisId", "pipeline", "trait", "gid", "designation", "mother", "father",
                           "entryType", "environment", "predictedValue", "stdError", "reliability")
    returnData$predictions <- provMet
  }
  if(is.null(returnData$modeling)){
    provMet <- as.data.frame(matrix(nrow=0, ncol=6))
    colnames(provMet) <- c("module", "analysisId", "trait", "environment", "parameter", "value")
    returnData$modeling <- provMet
  }
  ...
  for (i in (1:length(allFactor))) {
    if (is.numeric(data[,allFactor[i]])) {
      data[,allFactor[i]] <- factor(data[,allFactor[i]])
    } else {
      if (suppressWarnings(all(!is.na(as.numeric(as.character(factor(trimws(data[,allFactor[i]])))))))) {
        ...
        data[,allFactor[i]] <- factor(trimws(data[,allFactor[i]]))
      }
    }
  }

  if (!is.null(rep1)) {
    data[,rep1] <- factor(data[,rep1])
  }
  if (!is.null(rep2)) {
    data[,rep2] <- factor(data[,rep2])
  }
  ...
  for (i in (1:length(respvar))) {
    ...
    for (z in 1:nlevels(as.factor(data[,match(set, names(data))]))) {
      ...
      origData <- tempAllData[tempAllData[,set] == levels(as.factor(tempAllData[,match(set, names(tempAllData))]))[z],]

      # determine if the data set contains an observation
      if (length(na.omit(origData[,respvar[i]])) == 0) {
        stop("Error: The data set does not contain any observations.\n\n")
      }

      # converts to a factor
      for (y in (1:length(allFactor))) {
        origData[,allFactor[y]]     <- factor(origData[,allFactor[y]])
      }
      if (!is.null(rep1)) {
        origData[,rep1] <- factor(origData[,rep1])
      }
      if (!is.null(rep2)) {
        origData[,rep2] <- factor(origData[,rep2])
      }
        ...
        if (designChoice != 3 && designChoice != 6 && designChoice != 10 && designChoice != 14) {
          if (is.list(replications(formula, rawData))) {
            if (max(replications(formula, rawData)[[1]]) != 1) {
              cat("ERROR: Cannot perform ANOVA for balanced data. The data set for response variable '",
                   respvar[i],"' is unbalanced.\n\n", sep = "")
              next
            }
            ...
            if ((1 - (length(na.omit(newData[,respvar[i]]))/nrow(newData))) > 0.10) {
              cat("ERROR: Cannot perform ANOVA for balanced data for response variable '", respvar[i], "'.
      ...
      if (estimateData) {
        tempresult <- summary(suppressWarnings(aov(formula(mymodel), data = origData)))
      }
      ...
      if(inherits(result, what = "aovlist")){
      ...
      if (homogeneity || normality) {
        ...
        if (homogeneity) {
          bartlett.result <- STAR::HeteroskedasticityTest(data = assumpData, var = paste(names(assumpData)[2]), grp = "factor", method = c("bartlett"))
        ...
        if (normality) {
          if (nrow(assumpData) >= 3 && nrow(assumpData) <= 5000) {
            normality.result <- STAR::NormalityTest(data = assumpData, var = paste(names(assumpData)[2]), grp = NULL, method = c("swilk"))
      ...
      if (!is.null(sigEffect)) {
        if (!is.na(match(respvar[i], pwVar))) {
          for (j in (1:length(sigEffect))) {
            returnData <- STAR::pairwiseComparison(tempAnova[[i]][[z]], design, trimws(sigEffect[j]), data = tempData, respvar[i], environment = setLabel[z], pwTest, siglevel = sig, rdata = returnData, analysisId = aovAnalysisId)
          }
        } else {
          for (j in (1:length(sigEffect))) {
            returnData <- STAR::pairwiseComparison(tempAnova[[i]][[z]], design, trimws(sigEffect[j]), data = tempData, respvar[i], environment = setLabel[z], pwTest = NULL, siglevel = sig, rdata = returnData, analysisId = aovAnalysisId)
          }
        }
      }
  ...
  aovModeling <- NULL
  for(i in 1:4){
    j <- paste0("input$factor",i)
    if(!is.null(eval(parse(text=j)))){
      tempAovModeling <- data.frame(module="aov", analysisId=aovAnalysisId,
                                    ...
                                    value = paste(capture.output(dput(eval(parse(text=j)))), collapse = ""))
