...
# printAOVTable.default <- function(aovtable) {
	...
	colwidth <- max(c(nchar(trimws(rownames(aovtable), which = "right")), nchar("Source"))) + 2
     for (i in (1:4)) colwidth <- c(colwidth, max(c(nchar(tempaov[,i]), nchar(colnames(aovtable)[i])),na.rm = TRUE) + 2)
	...
	for (i in (1:nrow(aovtable))) {
	     cat(formatC(trimws(rownames(aovtable)[i], which = "right"), width = colwidth[1], format = "s", flag = "-"), sep = "")
